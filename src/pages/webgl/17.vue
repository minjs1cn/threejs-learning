...
function init() {
  ...
  float getEye(vec2 fragCoord, vec2 center, mat2 m) {
    vec2 vd = (fragCoord - (center + m *(fragCoord - center)));
  ...
  void main() {
    mat2 m1 = createM2(u_Time * 0.1);
    mat2 m2 = createM2(u_Time * 0.2);
    mat2 m3 = createM2(u_Time * 0.7);
    float a = getEye(gl_FragCoord.xy, vec2(u_CanvasSize.x * 0.25, u_CanvasSize.y * 0.75), m1);
    float b = getEye(gl_FragCoord.xy, vec2(u_CanvasSize.x * 0.5, u_CanvasSize.y * 0.25), m2);
    float c = getEye(gl_FragCoord.xy, vec2(u_CanvasSize.x * 0.75, u_CanvasSize.y * 0.5), m3);
    float f = 0.0;
    if (a + b + c > 1.0) {
      f = a * b * c;
    } else {
