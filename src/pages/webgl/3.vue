...
const tx = ref<HTMLInputElement>();
const ty = ref<HTMLInputElement>();
...
function init() {
  ...
  tx.value?.addEventListener("change", (e: any) => {
    translation[0] = Number(e.target.value);
    drawScene();
  });
  ty.value?.addEventListener("change", (e: any) => {
    translation[1] = Number(e.target.value);
    drawScene();
  });
  r.value?.addEventListener("change", (e: any) => {
    rotation[0] = Math.cos(e.target.value);
    rotation[1] = Math.sin(e.target.value);
    drawScene();
  });
}
onMounted(init);
...
<template>
  ...
  <div class="controller">
    <input type="range" ref="tx" />
    <input type="range" ref="ty" />
    <input type="range" ref="r" min="0" max="360" />
  </div>
</template>
